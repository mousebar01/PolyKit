...
export function AssetLibraryEntryCard({
  ...
  const content = (
    <>
      <span
        className={`relative flex size-12 shrink-0 items-center justify-center overflow-hidden rounded-md border transition-colors ${
      ...
      <span className="min-w-0 flex-1">
        <span className="flex min-w-0 items-center gap-1.5">
          <span className="min-w-0 truncate text-xs font-medium text-foreground" title={entry.displayName}>{entry.displayName}</span>
          {latest && <span className="shrink-0 rounded-full border border-sky-400/20 bg-sky-400/10 px-1.5 py-0.5 text-[9px] font-medium text-sky-300">{t('workflows.latest')}</span>}
        </span>
        <span className="mt-1 block min-w-0 truncate font-mono text-[10px] text-muted-foreground" title={entry.workspacePath}>{entry.workspacePath}</span>
      </span>
    </>
  )

  const footer = (
    <span className="mt-2.5 flex min-w-0 items-center justify-between gap-2 border-t border-divider pt-2">
      <span className="truncate text-[10px] font-medium text-muted-foreground">{typeLabel}</span>
      {actionLabel && (
