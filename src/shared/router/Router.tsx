...
function RouteLoading({ frame }: { frame: boolean }): JSX.Element {
  return (
    <div className={`flex flex-1 items-center justify-center overflow-hidden rounded-lg ${frame ? 'border border-border/35 bg-card/20' : 'bg-background'}`} role="status" aria-live="polite">
...
export default function Router(): JSX.Element {
  ...
  const frameClass = frame ? 'overflow-hidden rounded-lg border border-border/35 bg-card/20' : ''
