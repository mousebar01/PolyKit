...
export interface RpcSessionStartOptions {
  ...
  /** Optional server-owned session root, separate from Agent credentials/config. */
  sessionDir?: string;
}
...
export class AgentSessionWrapper {
  ...
  beginExtensionBinding(options: ExtensionBindingOptions = {}): void {
    void this.ensureExtensionsBound(options).catch((err) => {
      console.error("[polykit-agent] failed to dispatch session_start to extensions:", err instanceof Error ? err.message : err);
    });
  }
  ...
  private ensureExtensionsBound(options: ExtensionBindingOptions = {}): Promise<void> {
    ...
    this.extensionBindingPromise = (async () => {
      ...
      if (typeof this.inner.bindExtensions === "function") {
        ...
        await bindExtensions.call(this.inner, {
          ...
          shutdownHandler: () => this.emit({
            ...
            method: "notify",
            notifyType: "warning",
            message: "Extension requested shutdown, but shutdown is not supported in polykit-agent.",
      ...
      this.applyForcedEmptySystemPrompt();
      console.log(`[polykit-agent] session_start dispatched to extensions for session ${this.inner.sessionId}`);
    })().catch((err) => {
  ...
  private resetIdleTimer(): void {
    ...
    this.idleTimer = setTimeout(() => {
      ...
      void this.shutdown().catch((error) => {
        console.error("[polykit-agent] failed to shut down idle session:", error instanceof Error ? error.message : error);
      });
    }, 10 * 60 * 1000);
  }
    ...
    this.shutdownPromise = (async () => {
      try {
        try {
          ...
          console.error(
            "[polykit-agent] extension binding failed before session shutdown:",
  ...
  private createExtensionUiContext(): ExtensionUiContextLike {
    return {
      ...
      getAllThemes: () => [],
      getTheme: () => undefined,
      setTheme: () => ({ success: false, error: "Theme switching is not supported in polykit-agent extension UI yet" }),
...
export async function startRpcSession(
  ...
  const { toolNames, initialModel, thinkingLevel, sessionDir } = options;
  ...
  const starting = (async () => {
    ...
    if (toolNames !== undefined) {
      ...
      // tool registry — so they were unavailable in polykit-agent sessions even though the
    ...
    // extensions stay usable in polykit-agent just like in the `pi` CLI.
