...
// Short-TTL cache for the allowed-roots set. Without this, every scoped path
// operation re-scans every pi session on disk just to check access. 5s is
// short enough that newly-created cwds appear promptly; stored on globalThis
// so it survives Next.js hot-reload.
declare global {
...
export async function getAllowedFileRoots(): Promise<Set<string>> {
  ...
  for (const s of sessions) {
    ...
    // The project root (main repo shared by all worktrees) is available too —
