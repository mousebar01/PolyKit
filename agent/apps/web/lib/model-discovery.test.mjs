...
test("resolves environment-backed headers without an API key", async () => {
  process.env.POLYKIT_AGENT_DISCOVERY_TEST_TOKEN = "resolved-token";
  try {
    const auth = await resolveModelDiscoveryAuth("polykit-agent-header-only-test", {
      baseUrl: "https://example.invalid/v1",
      api: "openai-completions",
      headers: { "X-Discovery-Token": "$POLYKIT_AGENT_DISCOVERY_TEST_TOKEN" },
    });
    assert.equal(auth.apiKey, undefined);
    assert.deepEqual(auth.headers, { "X-Discovery-Token": "resolved-token" });
  } finally {
    delete process.env.POLYKIT_AGENT_DISCOVERY_TEST_TOKEN;
  }
});
