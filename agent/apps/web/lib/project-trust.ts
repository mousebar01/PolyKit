 ...
 *
 * polykit-agent *executes* project extensions when it builds session services: their
 * factory runs on import and their `session_start` handlers run on startup.
 * Without a trust gate, merely opening an untrusted repository in polykit-agent runs
 ...
 *
 * polykit-agent and the `pi` CLI share the same trust store. Projects with gated
