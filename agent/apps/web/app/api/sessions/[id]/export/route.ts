...
async function exportSession(filePath: string, outputPath: string): Promise<void> {
  ...
  // The fallback module lives next to the installed pi package. Keep this
  // runtime-only so webpack does not try to statically bundle an absolute URL.
...
export async function GET(
  ...
  try {
    ...
    const tempDir = join(tmpdir(), "polykit-agent-export");
