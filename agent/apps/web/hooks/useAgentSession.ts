...
import { agentApiPath, sendAgentCommand } from "@/lib/agent-client";
...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const loadSession = useCallback(async (sid: string, showLoading = false, includeState = false) => {
    ...
    try {
      ...
      try {
      const stateRes = await fetch(agentApiPath(`/sessions/${encodeURIComponent(sid)}/state`));
  ...
  const ensureNewSession = useCallback(async () => {
    ...
    const promise = (async () => {
      ...
      const res = await fetch(agentApiPath("/sessions"), {
        method: "POST",
  ...
  const connectEvents = useCallback((sid: string): Promise<EventStreamConnectionResult> => {
    closeEvents();
    const es = new EventSource(agentApiPath(`/sessions/${encodeURIComponent(sid)}/events`));
  ...
  const scheduleEventStreamClose = useCallback((sid: string) => {
    ...
    const checkServerIdle = async () => {
      ...
      try {
        const res = await fetch(agentApiPath(`/sessions/${encodeURIComponent(sid)}/state`));
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
    ...
    switch (event.type) {
      ...
      case "agent_end":
        ...
        if (sessionIdRef.current) {
          ...
          fetch(agentApiPath(`/sessions/${encodeURIComponent(sessionIdRef.current)}/state`))
            .then((r) => r.json())
  ...
  const loadModels = useCallback(async (signal?: AbortSignal) => {
    ...
    const modelsUrl = modelCwd ? agentApiPath(`/models?cwd=${encodeURIComponent(modelCwd)}`) : agentApiPath("/models");
