...
class McpEntrypointTests(unittest.TestCase):
    def test_entrypoint_exposes_observability_and_world_domain_bridges_without_agent_task_runtime(self) -> None:
        ...
        workflow_inspect = next(tool for tool in tools if tool.name == "polykit_workflow_inspect")
        self.assertIn("without changing the run", workflow_inspect.description)
