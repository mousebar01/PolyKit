...
_pymeshlab = None
_PYMESHLAB_AVAILABLE: bool | None = None
...
from services.asset_names import output_name as asset_output_name
...
def _require_pymeshlab():
    global _pymeshlab, _PYMESHLAB_AVAILABLE
    if _PYMESHLAB_AVAILABLE is None:
        try:
            import pymeshlab
...
def _resolve_input_path(raw_path: str) -> Path:
    ...
    if candidate.is_absolute():
        if os.environ.get("POLYKIT_HEADLESS") == "1":
            raise HTTPException(400, "Absolute paths are unavailable in headless mode; upload the asset first")
...
def optimize_mesh(body: OptimizeRequest):
    ...
    output_name = asset_output_name(stem, tag=f"opt{target_faces}")
    output_dir = input_path.parent if str(input_path).startswith(str(runtime_paths.workspace.resolve())) else runtime_paths.workspace / "Workflows"
...
def smooth_mesh(body: SmoothRequest):
    ...
    stem = input_path.stem
    output_name = asset_output_name(stem, tag=f"smooth{iterations}")
    output_dir = input_path.parent if str(input_path).startswith(str(runtime_paths.workspace.resolve())) else runtime_paths.workspace / "Workflows"
    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = output_dir / output_name
    result.export(str(output_path))
...
def transform_mesh(body: TransformRequest):
    ...
    stem = input_path.stem
    output_name = asset_output_name(stem, tag="xf")
    output_dir = input_path.parent if str(input_path).startswith(str(runtime_paths.workspace.resolve())) else runtime_paths.workspace / "Workflows"
    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = output_dir / output_name
...
async def import_mesh_by_path(body: ImportByPathRequest):
    if os.environ.get("POLYKIT_HEADLESS") == "1":
        raise HTTPException(400, "Local absolute-path imports are unavailable in headless mode")
    ...
    if ext == "ply" and _is_gaussian_ply(file_path):
        tmp_dir = tempfile.mkdtemp(prefix="polykit_import_")
...
def serve_file(path: str):
    ...
    if os.environ.get("POLYKIT_HEADLESS") == "1":
        try:
            file_path = resolve_workspace_path(runtime_paths.workspace, path)
        except ValueError:
            raise HTTPException(400, "Headless file paths must be workspace-relative")
    if not file_path.is_file():
...
def ply_to_splat(path: str):
    ...
    """
    workspace = runtime_paths.workspace.resolve()
    try:
        src = resolve_workspace_path(workspace, path)
    except ValueError:
        raise HTTPException(400, "Invalid path")
    if not src.is_file():
        raise HTTPException(404, "File not found")
    ...
    out = Path(tempfile.gettempdir()) / f"polykit_splat_{key}.splat"
    if not out.is_file():
        try:
...
def export_mesh(path: str, format: str):
    ...
    try:
        input_path = resolve_workspace_path(runtime_paths.workspace, path)
    except ValueError:
        raise HTTPException(400, "Invalid path")
