...
PolyKit 的核心原则是：**FastAPI 是权威运行时，React Web、CLI 和可选 MCP adapter 都是客户端。**

工作流执行、任务状态、可编辑工作流定义、Node Pack 注册、模型进程、World domain 和服务端工作区都由 FastAPI 统一管理。Web、CLI 和 MCP adapter 通过相同的 HTTP API 使用这些能力；MCP 不拥有第二套业务状态或执行引擎。
...
PolyKit 位于用户、自动化工具、外部 MCP 客户端、模型仓库和计算运行时之间：

```mermaid
flowchart LR
    User[User] -->|uses| PolyKit[PolyKit]
    Automation[CLI / Automation] -->|calls HTTP API| PolyKit
    MCPClient[External MCP Client] -->|stdio MCP adapter → HTTP| PolyKit
...
MCP adapter 是外部兼容层，不属于 FastAPI 内部执行路径。
...
FastAPI 既提供 API，也可以托管构建后的 Web 文件。CLI 与 MCP adapter 不复制产品逻辑，只调用相同的服务端合约。
...
flowchart TB
    Routers[API Routers]
    WorldDomain[World Domain + Validators]
    RunService[Workflow Run Service\nvalidation · lifecycle · GPU slot · observability]
...
World domain 负责 schema-v2 `WorldDocument`、BuildSpec/ScenePlan/GameSpec 以及确定性 quality validators；它不保存 Workflow stage/task progression。

## 4. Core Runtime Concepts

### Workflow

Workflow 是带类型约束的 DAG。可编辑定义通过 `/workflow-definitions/*` 保存，运行通过 `/workflow-runs/*` 提交、查询、inspect、重连和取消。定义本身不等于某一次运行。
...
一次工作流提交会创建一个可查询的 run。Run Service 负责校验、排队、生命周期、取消、GPU 执行槽和持久化 observability。Web、CLI 和外部 MCP client 都可以通过 run ID 查询状态；`GET /workflow-runs/{run_id}/inspect` 是只读观测接口，不会推进或重试任务。

### World

`WorldDocument` 保存世界本身的领域事实：intent、BuildSpec、ScenePlan、GameSpec、quality 与 artifact refs。运行过程属于 `WorkflowRun`，不写入 World stage/task state。

### MCP Adapter

`tools/polykit-mcp/server.py` 是一个无状态 stdio adapter：MCP `tools/call` 只转换成现有 HTTP API 请求。它不 import FastAPI domain services、不直接执行 Node Pack，也不持久化 Agent/session/task 状态。

开发时通过 MCP Inspector 测试协议与 tool schema；真实 Agent client 只用于最后验证模型是否能合理选择工具。
...
sequenceDiagram
    actor User
    participant Client as Web / CLI / MCP client
    ...
    User->>Client: request work
    Client->>API: POST /workflow-runs/execute
    API->>API: validate DAG, links and node types
    API->>Job: create and queue run
    API-->>Client: run_id
    Job->>Engine: execute DAG
    Engine->>Node: resolve and execute nodes
    Node->>Workspace: write intermediate artifact
    Engine->>Workspace: publish Output
    Client->>API: GET /workflow-runs/{run_id}/inspect
    API-->>Client: lifecycle + node/event/evidence timeline
```
...
src/areas/worlds/            World viewer / Three.js runtime
api/main.py                  FastAPI 应用和路由组合
api/routers/                 HTTP API 边界
api/services/                运行时、World domain、注册表和执行器
tools/polykit-cli/           JSON-first HTTP CLI
tools/polykit-mcp/           stateless external MCP adapter
...
- **Server is authoritative**：执行、状态、定义、World domain 和工作区持久化由 FastAPI 管理。
- **Web / CLI / MCP share the API**：客户端不复制另一套生成、World 或工作流逻辑。
- **MCP is an adapter, not a runtime**：MCP 只转换协议；不保存 Agent/task state，也不直接调 Node Pack。
...
- **Workflow observability is read-only**：inspection 描述执行，不决定执行。
