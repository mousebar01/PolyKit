...
- PolyKit 设置页已原生提供 `Agent` 子板块；这里只保留运行时开关和服务端会话目录，模型认证、技能、插件、MCP、归档会话和隐藏工作区则沿用 Agent 的本地配置边界并嵌入同一设置页。新会话的模型、思考级别和工具选择在对话页持久化。
...
| --- | --- |
| 新会话模型、思考级别和工具选择 | 对话页新会话控件，由 Agent 启动偏好持久化 |
