...
| `anima-trellis2-text-to-3d` | text → Anima → Trellis.2 → output | Stylized text-to-3D starting point |

Each template declares `requires: ["<pack-id>"]`. The editor lists templates separately and shows an install hint when a required pack is missing.
