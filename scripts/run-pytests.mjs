...
// Resolve an explicitly configured relative interpreter against the repository
// root before changing cwd to api/, otherwise `.venv/bin/python` becomes
// `api/.venv/bin/python` at execution time.
...
const configuredPython = process.env.POLYKIT_PYTHON
  ? (isAbsolute(process.env.POLYKIT_PYTHON) ? process.env.POLYKIT_PYTHON : resolve(rootDir, process.env.POLYKIT_PYTHON))
  : null
